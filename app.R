...
# Resolve the app's own directory so data files load correctly both locally
# and on shinyapps.io (which does not allow setwd() to absolute local paths).
# When run locally via RStudio, getSrcDirectory falls back to the script path.
APP_DIR <- tryCatch(
  dirname(rstudioapi::getSourceEditorContext()$path),
  error = function(e) getwd()   # shinyapps.io: working dir is already app folder
)
setwd(APP_DIR)

# Register www/ for static assets (images) relative to the app directory
addResourcePath("www", file.path(APP_DIR, "www"))
...
ui <- dashboardPage(
  ...
  # ---- 6a. Header -----------------------------------------------------------
  dashboardHeader(
    title = "EXPLORING THE DATA CENTER BOOM",
    titleWidth = 380
  ),
...
# =============================================================================
# DEPLOYMENT (ADMIN USE ONLY)
# =============================================================================

# options(rsconnect.quarto = FALSE)

# rsconnect::deployApp(
 # appDir        = "/Users/daniellestemper/Desktop/696 dash files",
 # appFiles      = c(
 #   "app.R",
 #   "www/mapping_ai_screenshot.png",
 #   "us_data_centers_geocoded.gpkg",
 #   "us_electricity_geocoded.gpkg",
 #   "SVI_2022_US_county.csv",
 #   "indiana_proposed_dc_2025_-_locs.csv"
 # ),
 # appPrimaryDoc = "app.R",
 # appName       = "DataCenterDash",
 # account       = "daniellestemp",
 # server        = "shinyapps.io",
 # forceUpdate   = TRUE
#)
